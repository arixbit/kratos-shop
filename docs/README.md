...
- [配置说明](configuration.md)：Demo 配置与 `config.local.yaml` 本地覆盖机制
...
- [P1 生产就绪可行性报告](feasibility-p1.md)：镜像发布、迁移、告警、K8s 等改造的决策记录
...
| `service/inventory/` | 库存 gRPC 服务（50055） |
| `service/payment/` | 支付 gRPC 服务（50056） |
| `sql/` | PostgreSQL 建表、Mock 数据与迁移 |
| `scripts/` | 初始化、迁移、备份恢复与 K8s 配置脚本 |
